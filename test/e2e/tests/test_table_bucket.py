...
class TestTableBucket:
    def test_create_update_delete(self, s3tables_client):
        ...
        # AWS-assigned status fields should be populated from GetTableBucket.
        assert cr["status"]["ownerAccountID"] is not None
        assert cr["status"]["tableBucketID"] is not None
        ...
        # Update 2: change tags (add one, remove one, keep one) and confirm the
        # controller reconciles via TagResource/UntagResource without looping.
        tag_updates = {
            "spec": {
                "tags": {
                    "environment": "prod",
